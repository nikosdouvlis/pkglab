...
- `pkglab listen` — start a coordinator that accepts publish signals from `pub --ping` and coalesces them into batched publish cycles. Per-workspace (uses Unix socket at `~/.pkglab/listeners/`). Each tag gets its own queue lane. Useful for watch-mode workflows where many packages rebuild simultaneously. `--verbose`/`-v` for detailed socket activity.
